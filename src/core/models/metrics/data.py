...
class MetricDataModel(AbstractBaseModel, CreatedUpdatedAtMixin):
    ...
    series_id = Column(
        Integer,
        ForeignKey("metric_series.id", name="metric_data_new_series_id_fkey", ondelete="CASCADE"),
        nullable=True,
        comment="Ссылка на серию данных",
    )

    period_id = Column(
        Integer, ForeignKey("metric_period.id", ondelete="CASCADE"), nullable=True, comment="Ссылка на период"
    )
    ...
    # Связи
    series = relationship("MetricSeriesModel", back_populates="data")
    period = relationship("MetricPeriodModel", back_populates="data_entries")
    country = relationship("CountryModel", back_populates="metric_data")
    city = relationship("CityModel", back_populates="metric_data")

    def __repr__(self):
        return f"<MetricDataModel(id={self.id}, series={self.series_id}, period={self.period_id})>"
