...
class CountryModel(AbstractBaseModel, CreatedUpdatedAtMixin):
    ...
    currency = Column(String(10), nullable=True, comment="Валюта страны")
    timezone = Column(String(50), nullable=True, comment="Часовой пояс страны")
    ...
    async def update(
        ...
        dto_get: CountryGetDTO,
        dto_update: CountryUpdateDTO,
        dto_options: Optional[CountryOptionsDTO] = None,
    ...
    @classmethod
    async def exists(cls, session: AsyncSession, dto_get: CountryGetDTO) -> bool:
    ...
    @classmethod
    async def delete(cls, session: AsyncSession, dto_get: CountryGetDTO) -> bool:
        """Удаление объекта CountryModel. Возвращает True, если объект найден и удалён, иначе False."""
